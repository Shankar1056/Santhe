...
public class InstitutionsAdapter extends RecyclerView.Adapter<InstitutionsAdapter.ViewHolder> {
    ...
    private List<InstitutionDataItem> locationOfficeDetails;
    private ClickOnReadmore clickOnReadmore;

    public InstitutionsAdapter(Context context, List<InstitutionDataItem> locationOfficeDetails, ClickOnReadmore clickOnReadmore) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        holder.tvCall.setText(this.locationOfficeDetails.get(position).getCollegePhone());
        holder.tvLoaction.setText(this.locationOfficeDetails.get(position).getCollegeAddress());
        holder.tvName.setText(this.locationOfficeDetails.get(position).getCollegeName());

        if (this.locationOfficeDetails.get(position).getCollegeAddress().length() >= 50) {

            String s = this.locationOfficeDetails.get(position).getCollegeAddress();
